function [ X, YP, Y , err ,merr ] = milmethrk4( f, a, b, n, y0 ,y)
%MILMETHRK4
...
for i = 1:3
    ...
    k3 = h*f(x+0.5*h,   y+ 0.5*(-1 + sqrt(2))*k1 + (1 -0.5*sqrt(2))*k2);
    k4 = h*f(x+h,   y-0.5*sqrt(2)*k2+ (1+0.5*sqrt(2))*k3);
    YP(i+1) = YP(i) + (1/6)*(k1+(2-sqrt(2))*k2+(2+sqrt(2))*k3+k4)*h; 
end
...
merr= max(err);
plot(X,YP,'r*',X,Y,'b');
title(['Metoda Milnea dla n=',num2str(n)]);
legend('Rozwi¹zanie znalezione metod¹ Milnea','Funkcja y');
end
